...
export function formatKstTimeRange(startDate, endDate) {
  return `${formatKstTime(startDate)} - ${formatKstTime(endDate)}`;
}
...
export function getKstDateRange(dateValue) {
  const date = String(dateValue || "").trim();

  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    throw new Error("Invalid KST date.");
  }

  const start = new Date(`${date}T00:00:00.000${KST_OFFSET}`);
  const end = new Date(start.getTime() + 24 * 60 * 60 * 1000);

  return { end, start };
}

export function getKstWeekRange(dateValue) {
  const date = String(dateValue || "").trim();
  ...
  const { start: dayStart } = getKstDateRange(date);
  const weekday = getKstWeekdayIndex(year, month, day);
